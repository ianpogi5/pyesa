...
Tests: `npm run test-api` (API smoke tests, `server/test/run.mjs`) plus two browser e2e suites:
...
npm run e2e                                       # set builder; BASE_URL / CHROMIUM_BIN env overrides available
npm run e2e-viewer                                # song viewer: fullscreen, auto-fit, pedal keys, swipe, auto-scroll pacing (needs only the dev server)
```

`npm run dev:https` serves over a self-signed cert (generated by `scripts/dev-cert.js` into the gitignored `certs/`). Needed to test anything gated on a secure context from a phone/tablet on the LAN — notably the screen wake lock, which is simply absent over `http://<lan-ip>`.
...
- `SongViewer` renders ChordPro via `chordsheetjs`: lyrics-only mode uses a custom renderer over the parsed song (skips chord/intro lines); chords mode uses `TextFormatter`. It is also the live-performance surface, replacing SongbookPro:
  - **Fullscreen** is a `fixed inset-0 z-[200]` overlay (above the `z-[100]` modals). Native `requestFullscreen` is only requested in a browser tab, never in a home-screen PWA (`isStandalone()`), because iPad Safari floats its own close button top-left over the title; in a tab a `NATIVE_FS_GUTTER` clears it. A screen wake lock is held while fullscreen (must be requested from the tap handler — Safari refuses it from an effect).
  - **Auto-fit**: on entering fullscreen the font is sized so the widest line fits on one row (measured off-screen per line, floored at `MIN_FIT_FONT`). `+`/`−` in fullscreen adjust a session-only `fsFontSize`; the persisted `fontSize` is for normal reading only. "Fit" hands back to auto.
  - **Auto-scroll** paces to the song's `Duration2` (SongbookPro's per-song length in seconds, present on ~70% of songs) via rAF; songs without one fall back to 20px/s. Travel is measured to the end of `.song-content`, not the YouTube embed.
  - **Foot pedal / keyboard**: one `window` keydown handler per viewer instance. Pedals are BT keyboards, so `pedal.forward`/`pedal.back` are key lists (defaults cover arrows, PageUp/Down, Space, Enter; "Learn" in the settings panel captures a pedal's actual key). Mode `page` scrolls a screen and advances at the end; `song` jumps. `e.repeat` is ignored so a held pedal cannot race. Shortcuts: F, S, C, +/−, Home, Esc.
  - **Duplicate-instance guard**: pages render the viewer twice (desktop + mobile) and modals can add a third, so a key press is only acted on by `isTopmostViewer()` — the instance whose own centre point `elementFromPoint` resolves into. Any new global listener in the viewer must go through the same guard or it fires twice.
  - Preferences (`lyricsOnly`, `fontSize`, `pedal`) persist under localStorage `pyesa-viewer-prefs`.
- `SetsPage` loads a set from IndexedDB first and refreshes from the network in the background. The refresh must never reset `currentSongIndex` (it keeps the current song by `Id`) and drops responses for a set the user has since left (`activeFilenameRef`) — on slow wifi that response can arrive minutes later, mid-performance. The service worker's `NetworkFirst` for `/files/*.json` has a 3s `networkTimeoutSeconds` for the same reason.
